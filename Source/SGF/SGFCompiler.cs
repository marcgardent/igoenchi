...
{		
	public enum TransformType 
		{None, HorizontalFlip, VerticalFlip, DiagonalFlip, CodiagonalFlip,
		 Rotate90, Rotate180, Rotate270, Random};
	...
	{					
		...
		private static Regex regEx = AntiUnixRegEx();
		...
		public static GoGame Compile(SGFTree gameTree, TransformType transform)
		{
			...
			else if (gameTree.ChildNodes.Count > 1)
			{
				throw new Exception("Game lists are not supported");
			}	
			
			var rootNode = gameTree.ChildNodes[0];
			foreach (SGFProperty property in rootNode.Properties)
			{
				if (property.Name == "SZ")
				{										
			...
			foreach (SGFProperty property in rootNode.Properties)
			{
				...
				if (property.Name == "KM")
				{
					try 
					{
						game.Info.Komi = ConfigManager.StringToFloat(property.Value);
					}
					catch (Exception)
					{
						game.Info.Komi = 0;
		...
		{	
			...
			while (singleBranch)
			{
				if (IsMoveNode(tree))
				{
					GoMoveNode node = new GoMoveNode(game.CurrentNode,
					                                 FindStone(tree),
					                                 game.MoveNumber % ConfigManager.BoardStorageStep == 0);
				...
				else
				{
					...
					foreach (SGFProperty property in tree.Properties)
					{
			...
			foreach (SGFProperty property in tree.Properties)
			{
				...
				switch (name)
				{
					...
					case "LB":
						...
						break;
					case "SQ": case "CR": case "TR": case "MA":	case "SL":
						node.EnsureMarkup();
						MarkType mark = 
							name == "SQ" ? MarkType.Square :
							name == "CR" ? MarkType.Circle :
							name == "TR" ? MarkType.Triangle :
							name == "MA" ? MarkType.Mark :
										   MarkType.Selected;
		...
		private static Stone FindStone(SGFTree tree)
		{
			foreach (SGFProperty property in tree.Properties)
			{
				...
				if (name == "B" || name == "W")
				{					
					Stone result = CompilePointValue(property.Value.ToLower());
		...
		private static TextLabel CompileLabelValue(string source)
		{
			try
			{
				...
				Stone stone = CompilePointValue(parts[0]);
